...
void* UploadBuffer::Map(void)
{
    ...
    auto read_range = CD3DX12_RANGE(0, m_BufferSize);
    m_pResource->Map(0, &read_range, &Memory);
    return Memory;
}

void UploadBuffer::Unmap(size_t begin, size_t end)
{
    auto read_range = CD3DX12_RANGE(begin, std::min(end, m_BufferSize));
    m_pResource->Unmap(0, &read_range);
}
